    ...
    public async Task<IReadOnlyList<FavoriteDto>> ListAsync(CancellationToken ct = default)
    {
        ...
        cmd.CommandText = """
            SELECT f.kind, f.entity_id,
                   COALESCE(b.title, p.title, d.title, s.title, a.title) AS title,
                   COALESCE(p.book_id, d.book_id, s.book_id, a.book_id) AS book_id,
                   f.created_at
            FROM favorite f
            ...
            WHERE f.user_id = $user
              AND COALESCE(b.title, p.title, d.title, s.title, a.title) IS NOT NULL
