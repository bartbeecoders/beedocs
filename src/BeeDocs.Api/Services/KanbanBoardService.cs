...
public sealed class KanbanBoardService(SqliteConnectionFactory db, ContentResolver resolver) : IKanbanBoardService
{
    ...
    public async Task<IReadOnlyList<KanbanBoardSummaryDto>> ListByBookAsync(string bookId, CancellationToken ct = default)
    {
        ...
        cmd.CommandText = """
            SELECT id, book_id, title, source, updated_at, card_count
            FROM kanban_board WHERE book_id = $book_id
        ...
        await using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = """
                UPDATE kanban_board SET title = $title, source = $source, content_ref = $content_ref,
                  content_size = $content_size, card_count = $card_count, updated_at = $updated_at
                WHERE id = $id
                """;
    ...
    private static KanbanBoardDto ToDto(KanbanBoard d) => new(
        Id: d.Id,
        BookId: d.BookId,
        Title: d.Title,
        Source: d.Source,
